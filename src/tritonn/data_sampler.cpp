...
UDINT rSampler::generateVars(rVariableList& list)
{
	...
	// Variables
	list.add(m_alias + ".method"      , TYPE::UINT , rVariable::Flags::___D, &m_method         , U_DIMLESS, ACCESS_SAMPLERS   , COMMENT::METHOD + m_flagsMethod.getInfo(true));
	list.add(m_alias + ".setup"       ,              rVariable::Flags::___D, &m_setup.Value    , U_DIMLESS, ACCESS_SETSAMPLERS, COMMENT::SETUP + m_flagsSetup.getInfo());
	list.add(m_alias + ".select"      ,              rVariable::Flags::___D, &m_select         , U_DIMLESS, ACCESS_SAMPLERS   , "Выбор бака:<br/>0 - емкость А<br/>1 - емкость Б");
	list.add(m_alias + ".command"     , TYPE::UINT , rVariable::Flags::____, &m_command        , U_DIMLESS, ACCESS_SAMPLERS   , COMMENT::COMMAND + m_flagsCommand.getInfo(true));
	list.add(m_alias + ".state"       , TYPE::UINT , rVariable::Flags::R___, &m_state          , U_DIMLESS, 0                 , COMMENT::STATUS + m_flagsState.getInfo(true));
	list.add(m_alias + ".noflow"      ,              rVariable::Flags::R___, &m_noflow         , U_DIMLESS, 0                 , "Флаг отсутствия расхода");
	list.add(m_alias + ".probe.period",              rVariable::Flags::___D, &m_probePeriod    , U_DIMLESS, ACCESS_SAMPLERS   , "Отбор по времени. Период отбора");
	list.add(m_alias + ".probe.volume",              rVariable::Flags::___D, &m_probeVolume    , U_DIMLESS, ACCESS_SAMPLERS   , "Отбор по объему. Требуемый объем для отбора");
	list.add(m_alias + ".probe.mass"  ,              rVariable::Flags::___D, &m_probeMass      , U_DIMLESS, ACCESS_SAMPLERS   , "Отбор по массе. Требуемая масса для отбора");
	list.add(m_alias + ".probe.test"  ,              rVariable::Flags::___D, &m_probeTest      , U_DIMLESS, ACCESS_SETSAMPLERS, "Количество тестовых доз");
	list.add(m_alias + ".grab.volume" ,              rVariable::Flags::R___, &m_grabVol        , U_ml     , 0                 , "Объем единичной дозы");
	list.add(m_alias + ".grab.count"  ,              rVariable::Flags::R___, &m_grabCount      , U_DIMLESS, 0                 , "Общее количество доз");
	list.add(m_alias + ".grab.present",              rVariable::Flags::R___, &m_grabPresent    , U_DIMLESS, 0                 , "Количество отобранных доз");
	list.add(m_alias + ".grab.remain" ,              rVariable::Flags::R___, &m_grabRemain     , U_DIMLESS, 0                 , "Количство оставшихся доз");
	list.add(m_alias + ".can.volume"  ,              rVariable::Flags::___D, &m_canVolume      , U_ml     , ACCESS_SETSAMPLERS, "Требуемый объем емкости");
	list.add(m_alias + ".can.present" ,              rVariable::Flags::R___, &m_canPresent     , U_ml     , 0                 , "Заполенный объем емкости");
	list.add(m_alias + ".can.remain"  ,              rVariable::Flags::R___, &m_canRemain      , U_ml     , 0                 , "Оставшийся для заполнения объем емкости");
	list.add(m_alias + ".interval"    ,              rVariable::Flags::R___, &m_interval       , U_DIMLESS, 0                 , "Интервал отбора");
	list.add(m_alias + ".time.remain" ,              rVariable::Flags::R___, &m_timeRemain     , U_msec   , 0                 , "Оставшееся время обора");
	list.add(m_alias + ".time.start"  ,              rVariable::Flags::R___, &m_timeStart      , U_sec    , 0                 , "Время старта пробоотбора");
	list.add(prefix  + "a.volume"     ,              rVariable::Flags::___D, &m_can[0].m_volume, U_ml     , ACCESS_SAMPLERS   , "Объем емкости А");
	list.add(prefix  + "b.volume"     ,              rVariable::Flags::___D, &m_can[1].m_volume, U_ml     , ACCESS_SAMPLERS   , "Объем емкости Б");
...
UDINT rSampler::generateMarkDown(rGeneratorMD& md)
{
	...
	md.add(this, false, rGeneratorMD::Type::CALCULATE)
			...
			.addXml (XmlName::TOTALS   , "object containing totals", false)
			.addXml (XmlName::RESERVE  , "sampler object", true)
			.addLink(XmlName::IOSTART  , true)
			.addLink(XmlName::IOSTOP   , true)
			...
			.addLink(XmlName::OVERFLOW_, true, "\t")
			.addLink(XmlName::FAULT    , true, "\t")
			.addLink(XmlName::WEIGHT   , true, "\t")
