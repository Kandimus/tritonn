...
UDINT rStation::calculate()
{
	...
	for (auto str : m_stream) {
		...
		if(!m_temp.isValid()) m_temp.m_value += (m_total.Inc.Mass > 0.0) ? str->getValue(XmlName::TEMP   , m_temp.m_unit, err) * (str->m_total.Inc.Mass / m_total.Inc.Mass) : 0.0;
		if(!m_pres.isValid()) m_pres.m_value += (m_total.Inc.Mass > 0.0) ? str->getValue(XmlName::PRES   , m_pres.m_unit, err) * (str->m_total.Inc.Mass / m_total.Inc.Mass) : 0.0;
		if(!m_dens.isValid()) m_dens.m_value += (m_total.Inc.Mass > 0.0) ? str->getValue(XmlName::DENSITY, m_dens.m_unit, err) * (str->m_total.Inc.Mass / m_total.Inc.Mass) : 0.0;
	}
...
//	list.add(m_alias + ".Setup"         , TYPE_UINT , rVariable::Flags::RS_, &m_setup.Value           , U_DIMLESS         , ACCESS_SA, );
	list.add(prefix + "present.volume"  , TYPE_LREAL, rVariable::Flags::R___, &m_total.Present.Volume  , m_unit.getVolume(), 0        , COMMENT::TOTAL_PRESENT + COMMENT::VOLUME);
	list.add(prefix + "present.volume15", TYPE_LREAL, rVariable::Flags::R___, &m_total.Present.Volume15, m_unit.getVolume(), 0        , COMMENT::TOTAL_PRESENT + COMMENT::VOLUME15);
	list.add(prefix + "present.volume20", TYPE_LREAL, rVariable::Flags::R___, &m_total.Present.Volume20, m_unit.getVolume(), 0        , COMMENT::TOTAL_PRESENT + COMMENT::VOLUME20);
	list.add(prefix + "present.mass"    , TYPE_LREAL, rVariable::Flags::R___, &m_total.Present.Mass    , m_unit.getMass()  , 0        , COMMENT::TOTAL_PRESENT + COMMENT::MASS);
	list.add(prefix + "inc.volume"      , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Inc.Volume      , m_unit.getVolume(), ACCESS_SA, COMMENT::TOTAL_INC     + COMMENT::VOLUME);
	list.add(prefix + "inc.volume15"    , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Inc.Volume15    , m_unit.getVolume(), ACCESS_SA, COMMENT::TOTAL_INC     + COMMENT::VOLUME15);
	list.add(prefix + "inc.volume20"    , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Inc.Volume20    , m_unit.getVolume(), ACCESS_SA, COMMENT::TOTAL_INC     + COMMENT::VOLUME20);
	list.add(prefix + "inc.mass"        , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Inc.Mass        , m_unit.getMass()  , ACCESS_SA, COMMENT::TOTAL_INC     + COMMENT::MASS);
	list.add(prefix + "raw.volume"      , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Raw.Volume      , m_unit.getVolume(), 0        , COMMENT::TOTAL_RAW     + COMMENT::VOLUME);
	list.add(prefix + "raw.volume15"    , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Raw.Volume15    , m_unit.getVolume(), 0        , COMMENT::TOTAL_RAW     + COMMENT::VOLUME15);
	list.add(prefix + "raw.volume20"    , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Raw.Volume20    , m_unit.getVolume(), 0        , COMMENT::TOTAL_RAW     + COMMENT::VOLUME20);
	list.add(prefix + "raw.mass"        , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Raw.Mass        , m_unit.getMass()  , 0        , COMMENT::TOTAL_RAW     + COMMENT::MASS);
	list.add(prefix + "past.volume"     , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Past.Volume     , m_unit.getVolume(), 0        , COMMENT::TOTAL_PAST    + COMMENT::VOLUME);
	list.add(prefix + "past.volume15"   , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Past.Volume15   , m_unit.getVolume(), 0        , COMMENT::TOTAL_PAST    + COMMENT::VOLUME15);
	list.add(prefix + "past.volume20"   , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Past.Volume20   , m_unit.getVolume(), 0        , COMMENT::TOTAL_PAST    + COMMENT::VOLUME20);
	list.add(prefix + "past.mass"       , TYPE_LREAL, rVariable::Flags::RSH_, &m_total.Past.Mass       , m_unit.getMass()  , 0        , COMMENT::TOTAL_PAST    + COMMENT::MASS);
