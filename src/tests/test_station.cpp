...
TEST_CASE("testing station calc.", "[Station]")
{
	SECTION("Check calculating flowrate. Linear KF") {
		...
		CHECK  (ss("sikn1.temperature.value")->getValueLREAL() == ss("var.test_sikn_temp")->getValueLREAL());
		CHECK  (ss("sikn1.pressure.value")->getValueLREAL() == (pres1 + pres2) / 2);
		CHECK  (ss("sikn1.density.value")->getValueLREAL() == (dens1 + dens2) / 2);
